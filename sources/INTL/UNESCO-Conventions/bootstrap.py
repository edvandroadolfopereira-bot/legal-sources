...
class UNESCOConventionsFetcher:
    ...
    def normalize(self, record: dict) -> Optional[Dict[str, Any]]:
        ...
        title = fields.get("title_en", "").strip()
        if not title:
            title = fields.get("title_fr", "").strip()
        if not title:
            return None

        # Build full text from all language versions
        text_parts = []
        intro_en = fields.get("introduction_en", "").strip()
        if intro_en:
            text_parts.append(intro_en)

        intro_fr = fields.get("introduction_fr", "").strip()
        if intro_fr and intro_fr != intro_en:
            text_parts.append(f"\n\n--- Version française ---\n\n{intro_fr}")

        intro_es = fields.get("introduction_es", "").strip()
        ...
        doc_id_num = fields.get("id", fields.get("uuid", ""))
        ...
        return {
            ...
            "title": title,
            "title_fr": fields.get("title_fr", ""),
            "title_es": fields.get("title_es", ""),
            "text": text,
